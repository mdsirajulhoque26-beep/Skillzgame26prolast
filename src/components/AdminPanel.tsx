...
export const AdminPanel: React.FC = () => {
  ...
  const removeTournament = async (t:any) => { if (!window.confirm(`“${t.name}” Tournament delete করবেন?\n\nযদি কোনো Player ইতিমধ্যে Join করে থাকে, নিরাপত্তার জন্য delete করা হবে না।`)) return; try { await backendApi.deleteTournament(t.id); if (selectedAdminTournament?.id === t.id) setSelectedAdminTournament(null); await refreshTournaments(); showToast('Tournament delete হয়েছে।'); } catch(e:any){showToast(e?.message||'Tournament delete করা যায়নি।','error');} };
